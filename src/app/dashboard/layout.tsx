...
const Layout: FC<LayoutProps> = async ({ children }: LayoutProps) => {
    ...
    return (<div className='w-full h-screen flex'>
        <div className='flex h-full w-full max-w-xs flex-col grow gap-y-5 overflow-y-auto border-r border-gray-200 bg-white px-6'>
            <Link href='/dashboard' className='h-16 flex shrink-0 items-center'><Icons.Logo className='h-8 w-auto text-indigo-600' /></Link>
            <div className='text-xs font-semibold leading-6 text-gray-400'>Your chats</div>
            <nav className='flex flex-1 flex-col'>
                <ul role='list' className='flex flex-1 flex-col gap-y-7'>
                    <SideBarChatList friends={friendsData} sessionId={session.user.id} />
        ...
        </div><aside className='max-h-screen container py-8 md:py-10 w-full'>{children}</aside></div>)
}
