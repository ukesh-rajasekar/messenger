...
const ChatInput: FC<ChatInputProps> = ({ chatPartner, chatId }) => {
    ...
    const isEmptyMsg = input === ''

    const firstName = chatPartner.name.split(' ')[0]
    ...
    return <div className='border-t border-gray-200 px-4 pt-4 mb-2 sm:mb-0'>
        <div className='relative flex-1 overflow-hidden rounded-lg shadow-sm ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-indigo-600'>
            ...
            />
            <div onClick={() => textareaRef?.current?.focus()} className='py-2' aria-hidden='true'><div className='py-px'>
                <div className='h-9' /></div></div>

            <div className='absolute right-0 bottom-0 flex justify-between py-2 pl-3 pr-2'>
                <div className='flex-shrin-0'>
                    <Button disabled={isEmptyMsg} isLoading={loading} onClick={sendMessage} type='submit'>send</Button>
                </div>
            </div>
        </div>
    </div>
