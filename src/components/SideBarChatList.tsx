...
const SideBarChatList: FC<SideBarChatListProps> = ({ friends, sessionId }) => {
   ...
   useEffect(() => {
      ...
      const newMessageHandler = (message: ExtendedMessage) => {
         const shouldNotify =
            pathName !=
            `/dashboard/chat/${chatLinkConstructor(
               sessionId,
               message.senderId
            )}`;
            ...
            return (
               <li key={friend.id}>
                  <a
                     ...
                     className='truncate text-gray-700 hover:text-indigo-600 hover:bg-gray-50 group flex gap-x-3 rounded-md p-2 text-md items-center'>
                     {friend.name}
                     {unseenMsgCount > 0 ? (
